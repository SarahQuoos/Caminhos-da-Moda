...
with st.expander("Conferir Fluxo de Caixa"):
    ...
    if st.button("Carregar dados de lucro"):
        ...
        listavendas['Mês/Ano Cadastro_str'] = listavendas['Mês/Ano Cadastro'].dt.strftime('%B/%Y')
        #listavendas['Mês/Ano Cadastro_str'] = listavendas['Mês/Ano Cadastro'].dt.strftime('%b/%Y').str.capitalize()
        pecas_mensais_aux = listavendas.groupby('Mês/Ano Cadastro_str')['Valor Pago na peça'].sum().reset_index()
